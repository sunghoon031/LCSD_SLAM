...
void FullSystem::printResult(
        std::string file,
        const int &nSkippedFrames,
        const int &nTotalFramesAfterInit,
        const double &trackingTimeMed,
        const double &trackingTimeAvg,
        const double &trackingTimeStd,
        const double &trackingStartTimestamp,
        const double &trackingEndTimestamp,
        const double &startTimestamp,
        const double &endTimestamp)
{
...
void FullSystem::printResultSeong(
        std::string file,
        const int &nSkippedFrames,
        const int &nTotalFramesAfterInit,
        const double &trackingTimeMed,
...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh)
{
	...
	else
	{
		...
		SE3 constant_mm = fh_2_slast.inverse() * lastF_2_slast;
		SE3 double_mm = fh_2_slast.inverse() * constant_mm;
		SE3 half_mm = SE3::exp(fh_2_slast.log()*0.5).inverse() * lastF_2_slast;


		// get last delta-movement.
        lastF_2_fh_tries.push_back(constant_mm);   // assume constant motion.
        lastF_2_fh_tries.push_back(double_mm);    // assume double motion (frame skipped)
        lastF_2_fh_tries.push_back(half_mm); // assume half motion.
        ...
        float rotDelta = setting_rotDelta;
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,rotDelta,0,0), Vec3(0,0,0)));            // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,0,rotDelta,0), Vec3(0,0,0)));          // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,0,0,rotDelta), Vec3(0,0,0)));          // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,-rotDelta,0,0), Vec3(0,0,0)));         // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,0,-rotDelta,0), Vec3(0,0,0)));         // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm* SE3(Sophus::Quaterniond(1,0,0,-rotDelta), Vec3(0,0,0)));          // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,rotDelta,rotDelta,0), Vec3(0,0,0)));   // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,0,rotDelta,rotDelta), Vec3(0,0,0)));   // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,rotDelta,0,rotDelta), Vec3(0,0,0)));   // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,-rotDelta,rotDelta,0), Vec3(0,0,0)));  // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,0,-rotDelta,rotDelta), Vec3(0,0,0)));  // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,-rotDelta,0,rotDelta), Vec3(0,0,0)));  // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,rotDelta,-rotDelta,0), Vec3(0,0,0)));  // assume constant motion.
        ...
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,0,-rotDelta,-rotDelta), Vec3(0,0,0))); // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,-rotDelta,0,-rotDelta), Vec3(0,0,0))); // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,-rotDelta,-rotDelta,-rotDelta), Vec3(0,0,0))); // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,-rotDelta,-rotDelta,rotDelta), Vec3(0,0,0)));  // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,-rotDelta,rotDelta,-rotDelta), Vec3(0,0,0)));  // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,-rotDelta,rotDelta,rotDelta), Vec3(0,0,0)));   // assume constant motion.
        lastF_2_fh_tries.push_back(constant_mm * SE3(Sophus::Quaterniond(1,rotDelta,-rotDelta,-rotDelta), Vec3(0,0,0)));  // assume constant motion.
...
void FullSystem::marginalizeEverythingByForce()
{
    if (!isLostSignalSent)
    {
        isLostSignalSent = true;
        for(IOWrap::Output3DWrapper* ow : outputWrapper)
        {
            ow->publishFullStop();
        }
    ...
    // Forcefully marginalize the remaining active Keypoints & Keyframes one by one.
    ...
    for(unsigned int i=0;i<frameHessians.size();i++)
    {
        if(frameHessians[i]->flaggedForMarginalization)
        {
            ...
            std::cout << "Marginalized points and KF by force" << std::endl;
...
void FullSystem::addActiveFrame( ImageAndExposure* image, int id, bool half_resolution )
{
	...
	if (half_resolution)
	    fh->makeImages(image->image_half, &Hcalib);
    else
...
void FullSystem::makeKeyFrame( FrameHessian* fh)
{
    ...
    for(IOWrap::Output3DWrapper* ow : outputWrapper)
    {
        ...
        if (setting_semiDirect) ow->publishDsoKeyframeStuff(frameHessians, false, &Hcalib);
    }
