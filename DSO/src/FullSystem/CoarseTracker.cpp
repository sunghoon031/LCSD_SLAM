...
CoarseTracker::CoarseTracker(int ww, int hh) : lastRef_aff_g2l(0,0)
{
	...
	idepth_present = allocAligned<4,float>(dso::wG[0]*dso::hG[0], ptrToDelete);
...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians)
{
    ...
    memset(idepth_present, 0, sizeof(float)*w[0]*h[0]);
...
//				// Seong Edit:
				if (new_idepth <= 0) continue;

				idepth[0][idx] += new_idepth*weight;
                weightSums[0][idx] += weight;
                idepth_present[idx] = 1;


//                // Seong: Try to suppress uncertain idepths near certain idepths. This helps rotation.
                if (setting_pointLocalSmooth)
                {
                    int pr = setting_patchRadius;
                    if (u-pr+w[0]*(v-pr) >=0 && u+pr+w[0]*(v+pr) < w[0]*h[0])
                    {
                        for (int ii = -pr; ii <= pr ; ii++)
                            for (int jj = -pr; jj <=pr ; jj++)
                            {
                                int new_idx = u+ii+w[0]*(v+jj);
                                int r2 = ii*ii+jj*jj;
                                int pr2 = pr*pr;

                                //float I_ref = lastRef->dIp[0][u+w[0]*v][0];
                                //float I_idx = lastRef->dIp[0][idx][0];

                                if (r2 > 0 && pr2 >= r2 )
                                {
                                    float factor = 1.0f - 0.5*r2/pr2; //between 0.5 (circumference) and 1 (center)
                                    float new_weight = weight*factor;
                                    idepth[0][new_idx] += new_idepth *new_weight;
                                    weightSums[0][new_idx] += new_weight;
	...
	int num_virtual_depths = 0;
	if (setting_pointLocalSmooth)
	{
	    for (int i = 0; i<w[0]*h[0]; i++)
            if (idepth_present[i] == 0)
            {
                    idepth[0][i] = 0;
                    weightSums[0][i] = 0;
            }
	...
	int lvl_dilation = 2;
	if (setting_halfResolution)
	    lvl_dilation = 1;


    // dilate idepth by 1.
    for(int lvl=0; lvl<lvl_dilation; lvl++)
    {
        ...
        for(int i=w[lvl];i<wh;i++)
        {
            ...
            {
                // [Seong note: I think 'x' shape patch]
	...
	for(int lvl=lvl_dilation; lvl<pyrLevelsUsed; lvl++)
	{
		int wh = w[lvl]*h[lvl]-w[lvl];
		int wl = w[lvl];
		float* weightSumsl = weightSums[lvl];
		...
		for(int i=w[lvl];i<wh;i++)
		{
			if(weightSumsl_bak[i] <= 0)
			{
			    // [Seong note: I think '+' shape patch]
				float sum=0, num=0, numn=0;
