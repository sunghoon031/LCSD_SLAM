...
{
    cv::Mat new_img = cv::Mat(img->h, img->w, CV_32F, img->data)*(1/254.0f);
	displayImage(windowName, new_img, autoSize);
}
void displayImage(const char* windowName, const MinimalImageF3* img, bool autoSize)
{
    cv::Mat new_img = cv::Mat(img->h, img->w, CV_32FC3, img->data)*(1/254.0f);
	displayImage(windowName, new_img, autoSize);
}
