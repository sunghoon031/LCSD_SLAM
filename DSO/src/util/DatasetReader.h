...
public:
	...
	void setGlobalCalibration(const bool &half_resolution)
	{
		int w_out, h_out;
		Eigen::Matrix3f K;
		getCalibMono(K, w_out, h_out);
		setGlobalCalib(w_out, h_out, K, half_resolution);
	}
	...
	//Seong addition:
	ImageAndExposure* getImageFromImageRaw(const MinimalImageB* minimg, int id)
    {
        return undistort->undistort<unsigned char>(
                minimg,
...
private:
	...
	inline void loadTimestamps()
	{
		...
		// SEONG: Adjust timestamps (if necessary)
		while (timestamps.at(1)-timestamps.at(0) >0.1)
		    std::transform(timestamps.begin(), timestamps.end(), timestamps.begin(),std::bind1st(std::multiplies<double>(),0.1));
		...
		//printf("first two timestamps: %f, %f \n", timestamps.at(0), timestamps.at(1));
	}
