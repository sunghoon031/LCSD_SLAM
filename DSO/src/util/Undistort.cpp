...
ImageAndExposure* Undistort::undistort(const MinimalImage<T>* image_raw, float exposure, double timestamp, float factor) const
{
	...
	// Seong Edit: Downsize the image by half:
    int h_half = result->h_half;
    int w_half = result->w_half;

	for(int y=0;y<h_half;y++)
        for(int x=0;x<w_half;x++)
        {
            result->image_half[x + y*w_half] = 0.25f * (result->image[2*x   + 2*y*w] +
                                                            result->image[2*x+1 + 2*y*w] +
                                                            result->image[2*x   + 2*y*w+w] +
                                                            result->image[2*x+1 + 2*y*w+w]);
        }
