...
public:
	...
	float* image;
	float* image_half;			// irradiance. between 0 and 256
	int w,h, w_half, h_half;				// width and height;
	double timestamp;
	...
	inline ImageAndExposure(int w_, int h_, double timestamp_=0) : w(w_), h(h_), timestamp(timestamp_)
	{
		...
		w_half = w/2;
		h_half = h/2;
		image_half = new float[w_half*h_half];
		exposure_time=1;
	}
	...
	inline ImageAndExposure* getDeepCopy()
	{
		...
		memcpy(img->image_half, image_half, w_half*h_half*sizeof(float));
		return img;
	}
};
