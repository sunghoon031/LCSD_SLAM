...
namespace dso
{
	...
	void setGlobalCalib(int w_in, int h_in,const Eigen::Matrix3f &K, bool half_resolution)
	{
	    int w = w_in;
	    int h = h_in;

	    // Seong edit: Downsize the image
	    if (half_resolution)
	    {
	        assert(w_in%2 == 0);
	        assert(h_in%2 == 0);
	        w = w_in/2;
	        h = h_in/2;
	        std::cout << "w & h after downsize = " << w << " & " << h << std::endl;
	        setting_halfResolution = true;
	    }
...
//			}
		...
		else
		{
	        fxG[0] = K(0,0)*0.5;
	        fyG[0] = K(1,1)*0.5;
	        cxG[0] = (K(0,2)+0.5)*0.5 - 0.5;
	        cyG[0] = (K(1,2)+0.5)*0.5 - 0.5;
	        KG[0]  << fxG[0], 0.0, cxG[0], 0.0, fyG[0], cyG[0], 0.0, 0.0, 1.0;  // synthetic
	        KiG[0] = KG[0].inverse();
	        fxiG[0] = KiG[0](0,0);
	        fyiG[0] = KiG[0](1,1);
	        cxiG[0] = KiG[0](0,2);
	        ...
	        std::cout << std::endl;
	        std::cout << "Downsized fx, fy, cx, cy = " << fxG[0] << ", " << fyG[0] << ", " << cxG[0] << ", "<< cyG[0] << std::endl;
	        std::cout << std::endl;
		}
